...
.autocomplete-suggestions {
    ...
    position: absolute; display: none; z-index: 9999; max-height: 254px; min-width: 350px; overflow: hidden; overflow-y: auto; box-sizing: border-box;
	}
...
<form class="form-horizontal" role="form" method="post" action="<?php echo Config::get('URL') . 'purchaseBook/saveOrder/';?>">
<div class="table-responsive">
	...
	<tr>
		...
		<td>
			<input class="customer-id<?php echo $i; ?>" name="customer_<?php echo $i; ?>" style="height: 38px; border: 0;" autocomplete="off" onclick="showCustomer(<?php echo $i; ?>)">
		</td>
		<td>
			<input class="pesanan-id<?php echo $i; ?>" name="pesanan_<?php echo $i; ?>" style="height: 38px; border: 0;" autocomplete="off" onclick="showMaterial(<?php echo $i; ?>)">
		</td>
		...
		<td>
			<input type="checkbox" name="lunas_<?php echo $i; ?>" value="1">
		</td>
...
// Autocomple contact
function showCustomer(rowNumber) {
	var classSelector = '.customer-id' + rowNumber;
...
// Autocomple contact
function showMaterial(rowNumber) {
	var classSelector = '.pesanan-id' + rowNumber;
	var customerID = new autoComplete({
	    selector: classSelector,
	    minChars: 1,
	    source: function(term, suggest){
