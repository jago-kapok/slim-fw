...
$no = 1;
$balance = 0;
$saldo_idr_trading_goods= 0;

foreach($this->trading_goods as $key => $value) {
	if ($value->quantity_stock > 0) {
		...
		echo '<td>' .  strtoupper($value->selling_currency) . '</td>';
		...
		$saldo_idr_trading_goods = $saldo_idr_trading_goods + $balance;
		echo '<td class="text-right">' . number_format($balance, 0) . '</td>';
		echo '<td class="text-right">' . number_format($saldo_idr_trading_goods, 0) . '</td>';
		echo "</tr>";
		$no++;
