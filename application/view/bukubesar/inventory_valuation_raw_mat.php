...
foreach($this->raw_mat as $key => $value) {
	if ($value->quantity_stock > 0) {
		...
		$saldo_idr_raw_mat = $saldo_idr_raw_mat + $balance;
		echo '<td class="text-right">' . number_format($balance, 0) . '</td>';
		echo '<td class="text-right">' . number_format($saldo_idr_raw_mat, 0) . '</td>';
		echo "</tr>";
		$no++;
