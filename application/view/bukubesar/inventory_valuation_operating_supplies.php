...
$no = 1;
$balance = 0;
$saldo_idr_operating_supplies= 0;
...
foreach($this->operating_supplies as $key => $value) {
	if ($value->quantity_stock > 0) {
		...
		$saldo_idr_operating_supplies = $saldo_idr_operating_supplies + $balance;
		echo '<td class="text-right">' . number_format($balance, 0) . '</td>';
		echo '<td class="text-right">' . number_format($saldo_idr_operating_supplies, 0) . '</td>';
		echo "</tr>";
		$no++;
