...
foreach($this->wip as $key => $value) {
	if ($value->quantity_stock > 0) {
		...
		echo '<td>' .  strtoupper($value->purchase_currency) . '</td>';
		...
		$saldo_idr_wip = $saldo_idr_wip + $balance;
		echo '<td class="text-right">' . number_format($balance, 0) . '</td>';
		echo '<td class="text-right">' . number_format($saldo_idr_wip, 0) . '</td>';
		echo "</tr>";
		$no++;
