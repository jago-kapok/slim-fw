...
<div class="collapse" id="changeDateRange">
  <div class="well">
    <form method="get" action="<?php echo Config::get('URL') . 'inventory/logInAll/'; ?>">
...
<?php $this->renderFeedbackMessages();?>
					...
					<tr>
					<th rowspan="2">No</th>
					<th class="align-center" rowspan="2">Tanggal</th>
					<th class="align-center" rowspan="2">Nama Barang</th>
					<th class="align-center" rowspan="2">#Transaction</th>
					<th class="align-center" colspan="2">Jumlah</th>
					<th class="align-center" colspan="2">Serial Number</th>
					</tr>
					<tr>
					<th class="align-center">Diterima</th>
					<th class="align-center">Ditolak</th>
					<th class="align-center">Diterima</th>
					<th class="align-center">Ditolak</th>
					</tr>
					</thead>
					...
					<tbody>
			          ...
			          foreach($this->log as $key => $value) {
                              ...
                              if ($tgl != $value->incoming_date) {
                                  echo '<td>' . date("d M, y", strtotime($value->incoming_date)) . '</td>';
                              } else {
                                echo '<td></td>';
                              }
	                          
	                          echo '<td>' . $value->material_name . '<br><a href="' . Config::get('URL') . 'inventory/editMaterial/?find=' . $value->material_code . '">' . $value->material_code . '</a></td>';
	                          echo '<td>' . $value->transaction_number . '</td>';
	                          echo '<td class="text-right">' . FormaterModel::trimTrailingZeroes($value->quantity_received) . '</td>';
	                          echo '<td class="text-right">' . FormaterModel::trimTrailingZeroes($value->quantity_reject) . '</td>';
	                          echo '<td>' . $value->serial_number . '</td>';
	                          echo '<td>' . $value->serial_number_received . '</td>';    	                          
	                          echo "</tr>";
                        $tgl = $value->incoming_date; 
                        $no++;
			          }
			          ?>
			          </tbody>
