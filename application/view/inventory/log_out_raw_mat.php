...
<div class="collapse" id="changeDateRange">
  <div class="well">
    <form method="get" action="<?php echo Config::get('URL') . 'inventory/logOutRawMat/'; ?>">
...
<?php $this->renderFeedbackMessages();?>
					...
					<tr>
  					...
  					<th class="align-center" rowspan="2">Jumlah</th>
            <th class="align-center" rowspan="2">Satuan</th>
            <?php
              if (Auth::isPermissioned('director,finance,purchasing')) {
                echo '<th class="align-center" colspan ="4" class="success">Harga Pembelian</th>';
              }
            ?>
					</tr>
          <?php
              if (Auth::isPermissioned('director,finance,purchasing')) {
                echo '<tr class="success">
                        <th class="align-center">#Lot Pembelian</th>
                        <th class="align-center">Harga</th>
                        <th class="align-center">Satuan</th>
                        <th class="align-center">Cur</th>
                      </tr>';
              }
            ?>
					</thead>
					...
					<tbody>
			          ...
			          foreach($this->log as $key => $value) {
                            ...
                            echo '<td>' . $value->material_out_unit . '</td>';
                            
                            if (Auth::isPermissioned('director,finance,purchasing')) {
                              echo '<td>' . $value->material_lot_number . '</td>';
                              echo '<td class="text-right">' . FormaterModel::trimTrailingZeroes($value->purchase_price). '</td>';
