<form method="post"  action="<?php echo Config::get('URL') . 'Po/approvePrBulk/'; ?>">
  <div id="PO" class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
      <thead>
        <tr class="warning">
          <th colspan="10" class="text-center">PERMINTAAN REVISI PO LAMA</th>     
        </tr>
        <tr class="warning">
          <th><INPUT type="checkbox" onchange="checkAll('edit-purchase-order-checkbox')" name="edit-purchase-order-checkbox" /></th>
      ...
      <tbody>
        ...
        $checkbox_number = 1;
        foreach($this->change_po_list as $key => $value) {
          ...
          if ($row > 1) {
            ...
            for ($i=1; $i <= $row; $i++) {
              ...
              if ($i === 1) {
                ...
                if ($total_purchase_per_po_number < $this->limit_approval->value OR $this->is_above_limit_approval) {
                  echo '<td rowspan="' . $row . '" class="text-center"><input name="change_transaction_number[' . $checkbox_number . ']' . '" type="checkbox" value="' . $value->transaction_number .'" class="edit-purchase-order-checkbox"></td>
                  ';
                } else {
        ...
        //check if director limit approval
        if ($total_purchase_per_po_number < $this->limit_approval->value OR $this->is_above_limit_approval) {
          echo '<td class="text-center"><input name="transaction_number[' . $checkbox_number . ']' . '" type="checkbox" value="' . $value->transaction_number .'" class="edit-purchase-order-checkbox"></td>';
        } else {
          echo '<td></td>';
        }
