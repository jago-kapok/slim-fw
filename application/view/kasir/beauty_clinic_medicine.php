<div class="tab">
    ...
    foreach($this->medicine_list as $key => $value) {
        if ($menu_category != $value->material_category) {
            ...
            echo '<button class="tab-item button-tab tablink ' . $style . '" onclick="openTab(event,\'medicine-' . $no .'\')">' . ucwords($value->material_category) . '</th>';
            $category_array['number'][] = 'medicine-' . $no;
    ...
    ?>
    <button class="tab-item button-tab tablink" onclick="openTab(event,'semua-medicine')">Semua</button>
</div>
<?php
...
<div id="semua-medicine" class="w3-container w3-border city" style="display:none">
