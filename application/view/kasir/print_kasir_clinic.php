...
<body>
  ...
  <div id="invoice-POS">
    ...
    <div id="mid">
        ...
        <p class="company-info">
          ...
          <?php 
            if (!empty($this->product[0]->customer_table_number) OR !empty($this->product[0]->customer_name)) {
              echo "<br>";
              if (!empty($this->product[0]->customer_name)) {
                echo "Customer: " . $this->product[0]->customer_name; 
              }
              if (!empty($this->product[0]->customer_table_number) AND !empty($this->product[0]->customer_name)) {
                echo " - Meja: " . $this->product[0]->customer_table_number;
              } elseif (!empty($this->product[0]->customer_table_number) AND empty($this->product[0]->customer_name)) {
                echo "Meja: " . $this->product[0]->customer_table_number;
              }
            }
          ?>
    ...
    <div id="sales-item">
            <table>
              <tr>
              <td>item</td>
              <td class="text-right padding-right">qty</td>
  ...
  <tr>
    <td colspan="4">
      Therapist: <?php echo $value->full_name;?>
    </td>
  </tr>
  </table>
<table id="total-sales">
  ...
  <tr>
    <td class="text-right padding-right">Pembayaran: </td>
    <td class="text-right"><?php echo number_format($value->received_payment,0); ?></td>
  </tr>
  
  <tr>
    <td class="text-right padding-right padding-bottom">Kembalian: </td>
    <td class="text-right padding-bottom"><?php echo number_format($value->payment_return,0); ?></td>
  </tr>
</table>
</div><!--End InvoiceBot-->

<div id="legalcopy">
  <p class="legal">Thanks for coming!<br>
  Wifi Username: <?php echo $company['wifi user name']; ?> Password: <?php echo $company['wifi password']; ?> 
  </p>
</div>
