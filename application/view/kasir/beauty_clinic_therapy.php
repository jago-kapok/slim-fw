<div class="tab">
    ...
    foreach($this->therapist_treatment_list as $key => $value) {
        if ($menu_category != $value->material_category) {
            ...
            echo '<button class="tab-item button-tab tablink ' . $style . '" onclick="openTab(event,\'therapy-' . $no .'\')">' . ucwords($value->material_category) . '</th>';
            $category_array['number'][] = 'therapy-' . $no;
    ...
    ?>
    <button class="tab-item button-tab tablink" onclick="openTab(event,'semua-therapy')">Semua</button>
</div>
<?php
...
<div id="semua-therapy" class="w3-container w3-border city" style="display:none">
