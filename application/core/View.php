...
class View
{
     ...
     */
    public function render($filename, $data = null, $theme = 'erp') //acebootstrap-3.3.7
    {
        if ($data) {
        ...
        if ($theme == 'erp') {
            if (Session::get('grade') >= 900) { 
                require Config::get('PATH_VIEW') . '_templates/' . $theme . '/header_all.php';
            } else {
                require Config::get('PATH_VIEW') . '_templates/' . $theme . '/header_' . Session::get('department') . '.php';
            }
        } else {
